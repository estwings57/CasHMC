...
static ConfigMap configMap[] =
{
	...
	DEFINE_PARAM(DOUBLE, BAND_SCALING),		DEFINE_PARAM(DOUBLE, PowPerLane),
	DEFINE_PARAM(DOUBLE, SleepPow),			DEFINE_PARAM(DOUBLE, DownPow),
	DEFINE_PARAM(DOUBLE, tPST),				DEFINE_PARAM(DOUBLE, tSME),
	DEFINE_PARAM(DOUBLE, tSS),				DEFINE_PARAM(DOUBLE, tSD),
	DEFINE_PARAM(DOUBLE, tSREF),			DEFINE_PARAM(DOUBLE, tOP),
	DEFINE_PARAM(DOUBLE, tQUIESCE),			DEFINE_PARAM(DOUBLE, tTXD),
	DEFINE_PARAM(DOUBLE, tRESP1),			DEFINE_PARAM(DOUBLE, tRESP2),
	DEFINE_PARAM(DOUBLE, tPSC),
		...
		if(field_name.compare(configMap[i].fieldName) == 0) {
			...
			case STRING: {
				...
				//LINK_POWER_MANAGEMENT
					...
					else if(field_value == "BANDWIDTH_AWARE") {
						LINK_POWER = BANDWIDTH_AWARE;
					}
					else if(field_value == "BANDWIDTH_SAT_CNT") {
						LINK_POWER = BANDWIDTH_SAT_CNT;
					}
					else {
						ERROR(" == Error - Unknown field_value ["<<field_value<<"] for field_name ["<<field_name<<"] in ini file");
						exit(0);
